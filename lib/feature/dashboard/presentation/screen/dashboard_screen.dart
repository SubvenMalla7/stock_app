...
class DashboardScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(body: LineChartSample2());
  }
}
...
List<Map<String, dynamic>> generateStockData(String view) {
  ...
  if (view == 'hourly') {
    for (int i = 0; i < 24; i++) {
      data.add({
        'date': DateTime(now.year, now.month, now.day, i).toString(),
        'price': (100 + random.nextDouble() * 400).toStringAsFixed(2),
      });
    }
  } else if (view == 'daily') {
    for (int i = 0; i < 30; i++) {
      data.add({
        'date': now.subtract(Duration(days: i)).toString().substring(0, 10),
        'price': (100 + random.nextDouble() * 400).toStringAsFixed(2),
      });
    }
    ...
    for (int i = 0; i < 5; i++) {
      final date = DateTime(now.year - i, now.month, now.day);
      data.add({
        'date': date.toString().substring(0, 4),
        'price': (100 + random.nextDouble() * 400).toStringAsFixed(2),
      });
    }
  }
